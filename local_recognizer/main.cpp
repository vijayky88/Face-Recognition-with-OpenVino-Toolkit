...
// --------------------------split--face--recog----style ------------------
        ...
        final_faces.clear();
        int num_x =frame.cols/300 ;
        int num_y =frame.rows/300 ;
...
//----------------------------------------face recognition--------------------------------------
        ...
        for (auto rect: detected_faces) {
           rectangle(frame, rect, cv::Scalar(111,219,212), 1) ;
        }

        int i=1;
        for (auto link:links){
            rectangle(boundary_face_frame,link.second, cv::Scalar(249,84,109),1);
